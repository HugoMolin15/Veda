...
void main() {
    float time = uTime;
    vec2 pos = vUv;
    float factor = uEnableWaves;
    
    float r = texture2D(uTexture, pos + cos(time * 2. - time + pos.x) * .01 * factor).r;
    float g = texture2D(uTexture, pos + tan(time * .5 + pos.x - time) * .01 * factor).g;
    float b = texture2D(uTexture, pos - cos(time * 2. + time + pos.y) * .01 * factor).b;
...
class CanvAscii {
    ...
    render() {
        ...
        const shaderMat = this.mesh.material as THREE.ShaderMaterial;
        shaderMat.uniforms.uTime.value = Math.sin(time);

        // Interpolate wave factor for smooth stop
        const targetWave = this.enableWaves ? 1.0 : 0.0;
        const currentWave = shaderMat.uniforms.uEnableWaves.value;
        shaderMat.uniforms.uEnableWaves.value += (targetWave - currentWave) * 0.05;
...
export default function ASCIIText({
    ...
    return (
        ...
        .ascii-text-container pre {
          ...
          left: 0;
          top: 0;
          background-image: radial-gradient(circle, #5227FF 0%, #7a69bf 50%, #a094c7 100%);
